...
def test_extract_names_from_text_success(monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["cli2.py", "sample2.txt"]) # "sample2.txt" will not be evaluated!
    ...
    with patch("name_extractor.cli2.extract_names_from_text", return_value=["Alice", "Bob"]):
        main()
...
def test_extract_names_from_json_success(monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["XXX", "sample.json", "--json"]) # argv[0] will not be evaluated in test!

    # Mocking extract_names_from_json
    with patch("name_extractor.cli2.extract_names_from_json", return_value=["Charlie", "David"]):
        main()
...
def test_file_not_found_text(monkeypatch, capsys):
    ...
    with patch("name_extractor.cli2.extract_names_from_text", side_effect=FileNotFoundError):
...
def test_file_not_found_json(monkeypatch, capsys):
    ...
    with patch("name_extractor.cli2.extract_names_from_json", side_effect=FileNotFoundError):
